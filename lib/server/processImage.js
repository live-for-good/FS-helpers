...
const needToResize = (originalSize, newSize) => {
  console.log("processImage 10")
  if ((!_has(originalSize, 'width') || !_has(originalSize, 'height'))
  && (_has(newSize, 'width') || _has(newSize, 'height'))) {
    console.log("processImage 13")
...
const processImage = (fileRef, processRequest) => new Promise((resolve) => {
  ...
  console.log("processImage 119", imageProcessingError)
} else {
            info.hasBeenResized = true;
            gm(newFilePath).identify((retMetadataError, metadata) => {
  console.log("processImage 123")
  if (retMetadataError) {
                // TODO LOG error log to collection
                // console.error(
